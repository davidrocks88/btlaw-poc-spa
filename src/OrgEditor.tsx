  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <div className="flex flex-col m-6">
        ...
        <hr className='my-4' />
        <label className="block text-gray-700 text-sm font-bold mb-2">Tags:
          <div className='my-2 flex flex-row flex-wrap gap-y-2 items-end align-middle content-center'>
            <div className={`border border-1 border-solid border-black ${tagCSS}`}>{
              creatingTag ? (
                <input type="text" className="bg-transparent" onChange={(event) => setNewTag(event.target.value)} onKeyPress={(event) => {
                  if (event.key === "Enter") {
                ...
                (
                  <div onClick={() => setCreatingTag(true)}><b>+</b> New Tag...</div>
                )
            }
            </div>
            {formik.values.tags.map(t => <div key={t} className="flex"><div className={tagCSS} onClick={() =>
              formik.setFieldValue('tags', formik.values.tags.filter(tag => tag !== t))
            }>{t}</div></div>)}
          </div>
        </label>


        <label className="block text-gray-700 text-sm font-bold mb-2">Available Tags:

          <div className='my-2 flex flex-row flex-wrap gap-y-2 items-end align-middle content-center'>
            {tags.length && tags.filter(t => !formik.values.tags.includes(t)).map(t => <div key={t} className="flex"><div className={tagCSS} onClick={() => formik.setFieldValue('tags', [...formik.values.tags, t])}>{t}</div></div>)}
          </div>
        </label>
