...
export function Migration() {
  ...
  async function migrateOrganizations() {
    if (organizations) {
      await Promise.all(
        organizations.map(async (legacyOrg) => {
          if (
            amplifyOrganizationList?.find(
              (org) => org.name === legacyOrg.name || org.id === legacyOrg.id,
            )
          ) {
            ...
            const tagsToMigrate = await Promise.all(
              legacyOrg.tags.map((legacyTagName) =>
                amplifyTags.find((amplifyTag) => amplifyTag.name === legacyTagName),
              ),
            )
            const input = {
              ...legacyOrg,
  ...
  return (
    <div className='flex flex-col'>
      <div>
        <button
          onClick={migrateTags}
          className='m-4 w-42 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'
        >
        ...
        <button
          onClick={migrateOrganizations}
          className='m-4 w-42 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'
        >
          Migrate Organizations
        </button>
        <button
          onClick={deleteAllOrganizations}
          className='m-4 w-42 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'
        >
      ...
      <div className='flex flex-row w-full'>
        <div>
          ...
          <ol className='list-decimal pl-10'>
            {tags.map((tag) => (
              <li className='font-mono' key={`tag-${tag}`}>
                {tag}
              </li>
            ))}
          </ol>
          ...
          <ol className='list-decimal pl-10'>
            {amplifyTags.map((tag) => (
              <li
                ...
                className='font-mono'
                key={`amplify-tag-${tag.id}`}
              >
                {JSON.stringify({ id: tag.id, name: tag.name })}
              </li>
            ))}
          </ol>
        </div>
        ...
        <div>
          <div className='text-xl font-bold'>
            Legacy Organizations - Total: {organizations?.length}
          </div>
          <ol className='list-decimal pl-10'>
            {organizations
              ?.sort((t1, t2) =>
                t1.name
                  .trim()
                  .toLocaleLowerCase()
                  .localeCompare(t2.name.trim().toLocaleLowerCase()),
              )
              .map((org) => (
                <li className='font-mono' key={`org-${org.name}`}>
                  {org.name}
                </li>
              ))}
          </ol>
        ...
        <div>
          <div className='text-xl font-bold'>
            Amplify Organizations - Total: {amplifyOrganizationList?.length}
          </div>
          <ol className='list-decimal pl-10'>
            {amplifyOrganizationList?.map((org) => (
              <li className='font-mono' key={`amplify-org-${org.id}`}>
